...
{
	// nezapomente povolit preruseni v systemu NVIC (inspirace viz 2. cviceni)
	NVIC_EnableIRQ(ADC0_IRQn); // ADC0_IRQn, hodnota 15

	// nastavovani ADC0

	// SC3
	ADC0_SC3 = (ADC_SC3_AVGS(3) | ADC_SC3_AVGE(1) | ADC_SC3_ADCO(1));

	// zdroj hodinoveho signalu: (Bus clock)/2, clock divider = 8
	// ADICLK - Input Clock Select, 01
	// ADIV - Clock Divide Select, 11
	ADC0_CFG1 |= ADC_CFG1_ADICLK(1);
	ADC0_CFG1 |= ADC_CFG1_ADIV(3);

	// rezim prevodu: kontinualni konverze, 8-bit,
	// MODE - It is single-ended 8-bit conversion, 00
	ADC0_CFG1 |= ADC_CFG1_MODE(0);

	ADC0_SC1A = (ADC_SC1_ADCH(6) | ADC_SC1_AIEN(1));
}
...
void ADC0_IRQHandler(void) {
	// zdroj hodnoty z prevodniku
	printDouble(ADC0_RA);
}

/**
 * https://stackoverflow.com/questions/12703307/%20*%20arm-none-eabi-gcc-printing-float-number-using-printf
 */
void printDouble(int value)
{
	double v = 3.3 * value  / 255.0;
	int intPart, fractPart;

	intPart = (int)v;
	fractPart = (int)((v - (int)v)*1000);

	if(fractPart < 0) fractPart *= -1;

	sprintf(result, "%1d.%03d\0", intPart, fractPart);
}
